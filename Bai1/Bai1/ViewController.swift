...
class ViewController: UIViewController {
    override func viewDidLoad() {
        super.viewDidLoad()
        xView1()
        xhinhTron(hinhTronWhite, 0.9, .white, 300, 100, _vienBo: 180)
        LBhi()
        bottonUser(btAccount, 0.85, 0.06, 650, .green, .white, "Create Account")
        bottonUser(btLogin, 0.85, 0.06, 720, .white , .black, "Log In")
    ...
    @objc func tele(){
        ...
        xhinhTron(hinhTronYellow, 0.05, .yellow, 350, 20, _vienBo: 10)
        xhinhTron1(hinhTronblue, 0.12, .blue, 300, 50, _vienBo: 25)
        xhinhTron(hinhTronPink, 0.25, .red, 500, 20, _vienBo: 50)
        b.append(hinhTronblue)
        b.append(hinhTronPink)
        b.append(hinhTronYellow)
        timer.invalidate()
        timer1 = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(tele1), userInfo: nil, repeats: true)
    }
    @objc func tele1(){
        for i in b {
            i.removeFromSuperview()
        ...
        b.removeAll()
        xhinhTron1(hinhTronYellow, 0.05, .yellow, 350, 20, _vienBo: 10)
        xhinhTron(hinhTronblue, 0.12, .blue, 300, 50, _vienBo: 25)
        xhinhTron1(hinhTronPink, 0.25, .red, 500, 20, _vienBo: 50)
        b.append(hinhTronblue)
        b.append(hinhTronPink)
        b.append(hinhTronYellow)
        timer1.invalidate()
        timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(tele), userInfo: nil, repeats: true)
    }
    func bottonUser(_ bottonbt: UIButton,_ widthbt: CGFloat,_ height: CGFloat,_ topAC: CGFloat,_ colorbt: UIColor,_ colortext: UIColor,_ text1: String){
        ...
        bottonbt.topAnchor.constraint(equalTo: view1.topAnchor, constant: topAC).isActive = true
    ...
    func xhinhTron(_ hTron: UIView,_ r: CGFloat,_ colorHT: UIColor,_ topAC: CGFloat,_ leftAC: CGFloat,_vienBo: CGFloat){
        ...
        hTron.layer.cornerRadius = CGFloat(_vienBo)
    }
    func xhinhTron1(_ hTron: UIView,_ r: CGFloat,_ colorHT: UIColor,_ topAC: CGFloat,_ leftAC: CGFloat,_vienBo: CGFloat){
        hTron.backgroundColor = colorHT
        view1.addSubview(hTron)
        hTron.translatesAutoresizingMaskIntoConstraints = false
        hTron.widthAnchor.constraint(equalTo: view1.widthAnchor, multiplier: r).isActive = true
